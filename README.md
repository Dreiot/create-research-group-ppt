...
- 公式优先使用可编辑数学对象；能力受限时使用 LaTeX/MathJax 矢量公式，不以代码式下划线文本替代数学排版。SVG 公式必须按 `viewBox` 等比缩放并居中，禁止横向或纵向拉伸。
- 正文使用中文术语，首次出现时可附英文全称和缩写。
- 先确认事实依据、冲突项和逐页大纲，再进入视觉制作；不把事实判断推迟到生成或验收阶段。
- 每次明确披露制作路径，可使用 `ppt-agent`、内置演示文稿工具、`oil-ppt` 组合流程或自定义流程。
- 默认只交付可编辑 PPTX；用户明确选择时只交付离线可编辑 HTML。
- 证据台账、页面渲染和质量检查仍须在内部完成，但默认使用临时目录且不保存到最终输出目录；仅在用户明确要求时保留。
...
显式选择离线 HTML 时，默认只交付 `<YYYY-MM-DD>_<汇报类型>.html`。页面证据清单、QA 报告、渲染图、源工程或检查数据仅在用户明确要求时保存。
